-- Generated from content/posts/winbridge-ui-roadmap.md
INSERT INTO posts (slug, title, description, content, date, tags, cover, series, series_order, featured, draft)
VALUES ('winbridge-ui-roadmap', 'WinBridge Issue #2：UI 与设置路线图——漂亮只是基础，状态必须看得懂', 'WinBridge v3.1.1 已经拥有完整 WPF 界面，下一阶段重点转向 DPI、可访问性、信息层级、日志可读性与跨窗口一致性。', 'WinBridge 仓库里的 **Issue #2** 专门跟踪 UI / Settings。
...
所以 Issue #2 计划让界面明确区分 Healthy fast path、Targeted repair、Full repair，还应该显示简短原因，例如 `Chrome latest target stale`、`Native Host registry missing`、`Computer Use runtime generation mismatch`。
...
未来 UI 希望把底层错误归类成 locked、stale、partial、protected-copy、Native Host、upstream runtime、unsupported policy，然后再告诉用户 WinBridge 可以修、只能诊断、建议等待上游，还是因为安全策略主动停止。
...
如果你遇到某个缩放比例文字被截断、键盘无法操作、阿拉伯语布局不对、日志卡顿，或者某个按钮“点了不知道有没有成功”，都可以直接在 #2 留下来。', '2026-08-14', '["WinBridge Recovery","WPF","UI","Accessibility","Windows","Issue"]', NULL, 'Windows 工具开发', 5, 0, 0)
